<div x-data="paymentAttempts()" class="m-0 md:m-2">
    <x-header :title="__('Payment Attempts')" :subtitle="__('Verify manual payment gateways and track attempts')" separator>
        <x-slot:middle class="!justify-end">
            <div class="flex items-center gap-2">
                <x-button
                    ...
                    {{ __('Filters') }}
                    @if($search || $itemStatus || $gateway)
                        <x-badge value="{{ ($search ? 1 : 0) + ($itemStatus ? 1 : 0) + ($gateway ? 1 : 0) }}" class="badge-primary badge-sm" />
                    @endif
                </x-button>
            </div>
        </x-slot:middle>
    ...
    <div x-show="filtersOpen"
        ...
        <x-card class="border-l-4 border-primary">
            <div class="flex items-center justify-between mb-4">
                ...
                @if($search || $itemStatus || $gateway)
                    <x-button
                        wire:click="$set('search', ''); $set('itemStatus', null); $set('gateway', null)"
                        icon="o-x-mark"
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                <div class="lg:col-span-2">
                    <x-input
                        ...
                        :label="__('Search')"
                        :placeholder="__('Search trx id, donors, or campaigns...')"
                ...
                <div>
                    <x-select
                        ...
                        :options="[
                            ...
                            ['id' => 'initiated', 'name' => __('Initiated')],
                            ['id' => 'pending', 'name' => __('Pending')],
                            ['id' => 'pending_verification', 'name' => __('Pending Verification')],
                ...
                <div>
                    <x-select
                        ...
                        :label="__('Gateway')"
                        :options="[
                            ['id' => null, 'name' => __('All Gateways')],
                            ['id' => 'bkash', 'name' => __('bKash')],
                            ['id' => 'nagad', 'name' => __('Nagad')],
                            ['id' => 'rocket', 'name' => __('Rocket')],
                            ['id' => 'aamarpay', 'name' => __('AamarPay')],
                            ['id' => 'shurjopay', 'name' => __('ShurjoPay')]
                        ]"
                        icon="o-credit-card" />
                </div>
                <div>
            ...
            <table class="table table-sm">
                <thead>
                    <tr class="text-center items-center font-semibold">
                        ...
                        <x-field :OB="$orderBy" :OD="$orderDirection" :field="'donation_id'">@lang('donation')</x-field>
                        <x-field :OB="$orderBy" :OD="$orderDirection" :field="'gateway'">@lang('gateway')</x-field>
                        <x-field :OB="$orderBy" :OD="$orderDirection" :field="'provider_reference'">@lang('trx id')</x-field>
                        ...
                        <x-field :OB="$orderBy" :OD="$orderDirection" :field="'created_at'">@lang('created')</x-field>
                ...
                <tbody>
                    @forelse($this->items as $item)
                        @php
                            ...
                            $manualGateway = in_array($item->gateway, ['bkash', 'nagad', 'rocket'], true);
                            $statusClass = match ($item->status) {
                                'success' => 'bg-emerald-100/60 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-300',
                                'pending', 'pending_verification', 'initiated' => 'bg-amber-100/60 text-amber-700 dark:bg-amber-900/20 dark:text-amber-300',
                        ...
                        <tr wire:key="item-{{ $item->id }}" id="item-id-{{ $item->id }}" class="text-center items-center" :class="{'bg-base-300 rounded-md': rows.includes('{{$item->id}}') }">
                            ...
                            <td class="max-w-48 truncate">
                                <p class="font-medium table-sm">{{ $donation?->campaign?->title ?? '-' }}</p>
                                <p class="text-xs opacity-60">{{ $donation?->donor_name ?? '-' }} · #{{ $donation?->id ?? '-' }}</p>
                            </td>
                            <td>
                                <div class="text-xs font-semibold">{{ strtoupper($item->gateway) }}</div>
                                <div class="text-[10px] opacity-60 uppercase">{{ $manualGateway ? __('manual') : __('auto') }}</div>
                            </td>
                            <td>
                                <div class="text-xs">{{ $item->provider_reference ?? '-' }}</div>
                                <div class="text-xs opacity-60">{{ $donation?->donor_email ?? __('Guest') }}</div>
                            </td>
                            <td>
                                <div class="text-xs">{{ number_format($item->amount, 0) }} {{ $item->currency }}</div>
                                <div class="text-xs opacity-60">{{ $donation?->status ? ucfirst($donation->status) : '-' }}</div>
                            </td>
                            <td>
                                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full {{ $statusClass }}">
                                    <span class="w-1.5 h-1.5 rounded-full bg-current"></span>
                                    <span class="text-xs font-normal">{{ Str::of($item->status)->replace('_', ' ')->title() }}</span>
                                </div>
                            </td>
                            <td>
                                <div class="text-xs">{{ $item->created_at?->format('M d, Y') ?? '-' }}</div>
                                <div class="text-xs opacity-60">{{ $item->completed_at?->format('M d, Y') ?? __('Not completed') }}</div>
                            </td>
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @can('payment-attempts.edit')
                                        <x-button
                                            ...
                                            icon="o-pencil"
                                            class="btn-ghost btn-sm"
                                            tooltip="{{ $manualGateway && $item->status === 'pending_verification' ? __('Verify') : __('Edit') }}" />
                                    @endcan
                                    @can('payment-attempts.delete')
                                        <x-button
                                            @click="confirmDelete({{ $item->id }}, '{{ addslashes($item->provider_reference ?? 'Payment Attempt') }}')"
                                            icon="o-trash"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-12 text-center text-base-content/60">
                                ...
                                <div>{{ __('No payment attempts found') }}</div>
                                @if($search || $itemStatus || $gateway)
                                    <x-button
                                        wire:click="$set('search', ''); $set('itemStatus', null); $set('gateway', null)"
        ...
        <div @click.stop
            ...
            <form @submit.prevent="$wire.editData()">
                <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    @php
                        $detailDonation = $paymentAttempt?->donation;
                        $detailGateway = $paymentAttempt?->gateway;
                    @endphp

                    <div class="grid gap-4 md:grid-cols-3">
                        <div class="p-4 border rounded-lg dark:border-gray-700">
                            <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Campaign') }}</p>
                            <p class="mt-1 font-semibold text-gray-900 dark:text-white">{{ $detailDonation?->campaign?->title ?? '-' }}</p>
                            <p class="text-xs opacity-60">#{{ $detailDonation?->campaign_id ?? '-' }}</p>
                        </div>
                        <div class="p-4 border rounded-lg dark:border-gray-700">
                            <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Donor') }}</p>
                            <p class="mt-1 font-semibold text-gray-900 dark:text-white">{{ $detailDonation?->donor_name ?? '-' }}</p>
                            <p class="text-xs opacity-60">{{ $detailDonation?->donor_email ?? __('Guest') }}</p>
                        </div>
                        <div class="p-4 border rounded-lg dark:border-gray-700">
                            <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Amount') }}</p>
                            <p class="mt-1 font-semibold text-gray-900 dark:text-white">{{ $detailDonation?->amount ? number_format($detailDonation->amount, 0) : '-' }} {{ $detailDonation?->currency ?? 'BDT' }}</p>
                            <p class="text-xs opacity-60">{{ __('Donation status') }}: {{ $detailDonation?->status ? ucfirst($detailDonation->status) : '-' }}</p>
                        </div>
                    </div>
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <x-input
                            wire:model.defer="provider_reference"
                            :label="__('Transaction ID')"
                            :placeholder="__('Enter trx id')"
                            icon="o-document-text" />
                    </div>

                    <div class="grid gap-4 md:grid-cols-2">
                        <x-input
                            wire:model.defer="completed_at"
                            type="datetime-local"
                            :label="__('Completed At')"
                            icon="o-calendar" />

                        <div class="p-4 border rounded-lg dark:border-gray-700">
                            <p class="text-xs uppercase tracking-wide text-gray-500">{{ __('Gateway') }}</p>
                            <p class="mt-1 font-semibold text-gray-900 dark:text-white">{{ $detailGateway ? strtoupper($detailGateway) : '-' }}</p>
                            <p class="text-xs opacity-60">{{ $paymentAttempt?->status ? Str::of($paymentAttempt->status)->replace('_', ' ')->title() : '-' }}</p>
                        </div>
                    </div>
                </div>
...
<script>
    Alpine.data('paymentAttempts', () => ({
        ...
        confirmDelete(id, title = 'Delete Payment Attempt') {
        ...
        confirmDeleteMultiple(count) {
            ...
            this.confirmMessage = `{{ __("Are you sure you want to delete") }} ${count} {{ __("selected payment attempts?") }} {{ __("This action cannot be undone.") }}`;
            this.confirmType = 'danger';
            this.confirmAction = () => {
                $wire.deleteMultiple();
                this.closeConfirm();
