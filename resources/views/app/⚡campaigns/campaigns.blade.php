<div x-data="campaigns()" class="m-0 md:m-2">
    <x-header :title="__('Campaigns')" :subtitle="__('Manage your donation campaigns')" separator>
            ...
            <div class="flex items-center gap-2">
                ...
                @can('campaigns.create')
                    <x-button @click="toggleModal" icon="o-plus" class="btn-primary btn-sm">
                        <span class="hidden sm:inline">{{ __('Add Campaign') }}</span>
            ...
            <table class="table table-sm">
                <thead>
                    <tr class="text-center items-center font-semibold">
                      ...
                      <x-field :OB="$orderBy" :OD="$orderDirection" :field="'goal_amount'">@lang('goal')</x-field>
                      <x-field :OB="$orderBy" :OD="$orderDirection" :field="'status'">@lang('status')</x-field>
                      <x-field :OB="$orderBy" :OD="$orderDirection" :field="'starts_at'">@lang('starts')</x-field>
                      <x-field>@lang('donations')</x-field>
                      <x-field>@lang('expense')</x-field>
                ...
                <tbody>
                    @forelse($this->items as $item)
                        @php
                            ...
                            $expense = (float) ($item->expenses_sum ?? 0);
                        ...
                        <tr wire:key="item-{{ $item->id }}" id="item-id-{{ $item->id }}" class="text-center items-center" :class="{'bg-base-300 rounded-md': rows.includes('{{$item->id}}') }">
                            ...
                            <td>
                                <div class="text-xs">৳ {{ number_format($item->goal_amount, 0) }}</div>
                                <div class="text-xs opacity-60">{{ $item->paid_donations_count ?? 0 }} paid</div>
                            </td>
                            <td>
                                <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full {{ $item->status == 'active' ? 'bg-emerald-100/60 dark:bg-emerald-900/20' : ($item->status == 'closed' ? 'bg-amber-100/60 dark:bg-amber-900/20' : 'bg-gray-100 dark:bg-gray-700') }}">
                                    <span class="w-1.5 h-1.5 rounded-full {{ $item->status == 'active' ? 'bg-emerald-500' : ($item->status == 'closed' ? 'bg-amber-500' : 'bg-gray-500') }}"></span>
                                    <button type="button"
                                            wire:click="changeStatus({{ $item->id }})"
                                            class="text-xs font-normal cursor-pointer {{ $item->status == 'active' ? 'text-emerald-600 dark:text-emerald-400' : ($item->status == 'closed' ? 'text-amber-600 dark:text-amber-400' : 'text-gray-600 dark:text-gray-400') }}">
                                        {{ ucfirst($item->status) }}
                                    </button>
                                </div>
                            </td>
                            <td>
                                <div class="text-xs">{{ $item->starts_at?->format('M d, Y') ?? '-' }}</div>
                                <div class="text-xs opacity-60">{{ $item->ends_at?->format('M d, Y') ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="text-xs">৳ {{ number_format($raised, 0) }}</div>
                                <div class="text-xs opacity-60">Balance ৳ {{ number_format($balance, 0) }}</div>
                            </td>
                            <td>
                                <div class="text-xs">৳ {{ number_format($expense, 0) }}</div>
                                <div class="text-xs opacity-60">{{ $item->paid_donations_count ?? 0 }} donors</div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="py-12 text-center text-base-content/60">
                                <x-icon name="o-flag" class="w-12 h-12 mx-auto mb-2 opacity-20" />
                                <div>{{ __('No campaigns found') }}</div>
                                @if($search || $itemStatus)
                                    <x-button
                                        wire:click="$set('search', ''); $set('itemStatus', null)"
        ...
        <div @click.stop
            ...
            <div class="flex items-center justify-between p-6 border-b dark:border-gray-700">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    <span x-show="!editMode">{{ __('Add Campaign') }}</span>
                    <span x-show="editMode">{{ __('Edit Campaign') }}</span>
                </h3>
            ...
            <form @submit.prevent="editMode ? $wire.editData() : $wire.saveData()">
                <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <x-textarea
                        ...
                        rows="4"
                        :placeholder="__('What is this campaign about?')" />

                    <div class="grid gap-4 md:grid-cols-3">
                        <x-input
                            wire:model.defer="goal_amount"
                            type="number"
                            :label="__('Goal Amount (BDT)')"
                    ...
                    <div x-data="{ imageOpen: true }" class="border rounded-lg dark:border-gray-700">
                        ...
                        <div x-show="imageOpen"
                            ...
                            <x-avatar-upload
                                ...
                                :image="$photo ? $photo->temporaryUrl() : getImage($campaign, 'cover', 'thumb')"
                ...
                <div class="flex items-center gap-3 p-6 border-t dark:border-gray-700">
                    <x-button
                        ...
                        class="btn-primary"
                        icon="o-check">
                        <span x-show="!editMode">{{ __('Create Campaign') }}</span>
                        <span x-show="editMode">{{ __('Update Campaign') }}</span>
                    </x-button>
                    <x-button
                        wire:loading
                        wire:target="editData,saveData"
...
<script>
    Alpine.data('campaigns', () => ({
        ...
        confirmDelete(id, title = 'Delete Campaign') {
        ...
        confirmDeleteMultiple(count) {
            ...
            this.confirmMessage = `{{ __("Are you sure you want to delete") }} ${count} {{ __("selected campaigns?") }} {{ __("This action cannot be undone.") }}`;
            this.confirmType = 'danger';
            this.confirmAction = () => {
                $wire.deleteMultiple();
                this.closeConfirm();
