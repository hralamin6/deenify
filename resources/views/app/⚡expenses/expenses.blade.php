<div x-data="expenses()" class="m-0 md:m-2">
    <x-header :title="__('Expenses')" :subtitle="__('Track campaign expenses')" separator>
            ...
            <table class="table table-sm">
                <thead>
                    <tr class="text-center items-center font-semibold">
                      ...
                      <x-field :OB="$orderBy" :OD="$orderDirection" :field="'spent_at'">@lang('spent')</x-field>
                      <x-field>@lang('category')</x-field>
                      <x-field>@lang('receipt')</x-field>
                ...
                <tbody>
                    @forelse($this->items as $item)
                        @php
                            $receiptUrl = getImage($item, 'receipt', null, null);
                        ...
                        <tr wire:key="item-{{ $item->id }}" id="item-id-{{ $item->id }}" class="text-center items-center" :class="{'bg-base-300 rounded-md': rows.includes('{{$item->id}}') }">
                            ...
                            <td class="max-w-48 truncate">
                                ...
                                <p class="text-xs opacity-60">{{ Str::limit($item->description ?? '', 40) }}</p>
                            </td>
                            <td>
                                <div class="text-xs">৳ {{ number_format($item->amount, 0) }}</div>
                            </td>
                            <td>
                                <div class="text-xs">{{ $item->spent_at?->format('M d, Y') ?? '-' }}</div>
                            ...
                            <td>
                                @if($hasReceipt)
                                    <a class="link link-primary text-xs" href="{{ $receiptUrl }}" target="_blank">
                                        {{ __('View') }}
                                    </a>
                                @else
                                    <span class="text-xs opacity-60">{{ __('No receipt') }}</span>
                                @endif
                            </td>
                            <td class="text-center items-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @can('expenses.delete')
                                        <x-button
                                            @click="confirmDelete({{ $item->id }}, '{{ addslashes($item->campaign?->title ?? 'Expense') }}')"
                                            icon="o-trash"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="py-12 text-center text-base-content/60">
                                <x-icon name="o-receipt-percent" class="w-12 h-12 mx-auto mb-2 opacity-20" />
                                <div>{{ __('No expenses found') }}</div>
                                @if($search || $itemStatus)
                                    <x-button
                                        wire:click="$set('search', ''); $set('itemStatus', null)"
        ...
        <div @click.stop
            ...
            <div class="flex items-center justify-between p-6 border-b dark:border-gray-700">
                <h3 class="text-xl font-semibold text-gray-900 dark:text-white">
                    <span x-show="!editMode">{{ __('Add Expense') }}</span>
                    <span x-show="editMode">{{ __('Edit Expense') }}</span>
                </h3>
            ...
            <form @submit.prevent="editMode ? $wire.editData() : $wire.saveData()">
                <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <x-select
                            ...
                            :label="__('Category')"
                            icon="o-tag"
                            :options="$this->categories->map(fn ($c) => ['id' => $c->id, 'name' => $c->name])->toArray()"
                    ...
                    <div class="border rounded-lg dark:border-gray-700">
                        <div class="flex items-center justify-between w-full p-4 text-left">
                            <div class="flex items-center gap-2">
                                <x-icon name="o-paper-clip" class="w-5 h-5" />
                                <span class="font-medium">{{ __('Receipt Attachment') }}</span>
                            </div>
                        </div>
                        <div class="p-4 space-y-4 border-t dark:border-gray-700">
                            <x-input
                                wire:model="receipt"
                                type="file"
                                :label="__('Upload Receipt')"
                                :hint="__('PDF, JPG, PNG up to 4MB')" />

                            @if($expense)
                                <div class="flex items-center gap-3 text-sm">
                                    <span class="opacity-60">{{ __('Current receipt') }}:</span>
                                    <a class="link link-primary" href="{{ getImage($expense, 'receipt', null, null) }}" target="_blank">
                                        {{ __('View') }}
                                    </a>
                                </div>
                            @endif
                ...
                <div class="flex items-center gap-3 p-6 border-t dark:border-gray-700">
                    <x-button
                        ...
                        class="btn-primary"
                        icon="o-check">
                        <span x-show="!editMode">{{ __('Create Expense') }}</span>
                        <span x-show="editMode">{{ __('Update Expense') }}</span>
                    </x-button>
                    <x-button
                        wire:loading
                        wire:target="editData,saveData"
...
<script>
    Alpine.data('expenses', () => ({
        ...
        confirmDelete(id, title = 'Delete Expense') {
        ...
        confirmDeleteMultiple(count) {
            ...
            this.confirmMessage = `{{ __("Are you sure you want to delete") }} ${count} {{ __("selected expenses?") }} {{ __("This action cannot be undone.") }}`;
            this.confirmType = 'danger';
            this.confirmAction = () => {
                $wire.deleteMultiple();
                this.closeConfirm();
